...
double getNNDist(double *ts, int tsLen, double *query, int sLen, double *zQuery, double *zWin) {
	...
	double *win, dist, nnDist = INF;	//the sliding window
...
int main(int argc, char **argv) {
	...
	std::string fName = path + "\\" + datasetName + "_PUSh_shInfo_" + s_seedId + "_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	...
	//Output
	std::ofstream fout;
	fName = path + "\\" + datasetName + "_PUSh_test_performance_" + s_seedId + +"_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	fout.open(fName);

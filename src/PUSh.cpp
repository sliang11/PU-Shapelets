...
typedef std::tuple<std::vector<int>, std::vector<double>, std::vector<int>> vecTuple;
...
void getShapeletPrecision(double &sp, double &spGap, double *distVec, int *order, int *preLabels_g, int *preLabels_l, int *preLabels_r, int numTrain, int numPLabeled, int iter) {
	...
	if (sp_l > sp_r) {
		sp = sp_l;
		spGap = distVec[order[curPreNumP]] - distVec[order[curPreNumP - 1]];
	}
	else {
		sp = sp_r;
		spGap = distVec[order[numTrain - curPreNumP]] - distVec[order[numTrain - curPreNumP - 1]];
	}
}
...
int main(int argc, char **argv) {
	...
	for (iter = 0; iter < maxNumIters; iter++) {
		...
		for (shIdTotal = 0; shIdTotal < numShTotal; shIdTotal++) {
			...
			//infoGain(gainComplete[shIdTotal], splitPt, secondaryComplete[shIdTotal], distVec, preLabels, order, numByLabel, numByLabelIn, numByLabelOut, numTrain, 2);
		}
	...
	int *curSpStopIterBelow = (int *)malloc(minIter * sizeof(int));
	...
	for (int i = 0; i < maxNumSh; i++) {
		...
		if (maxSpBelow >= maxSps[i]) {
			for (int j = 0; j < numMaxSpBelow; j++) {
				iter = curSpStopIterBelow[j];
				if (curAvgSpByIter[iter] >= maxSps[i]) {
					lastIterBelow = iter; //The last stopping point before minIter with max ASP
				}
			}
		}
		else
				...
				if (gap >= gapTh) {
					if (j - 1 == 0) {
						if (lastIterBelow == -1)
							...
							prevGap = curSpStopIter[0] - lastIterBelow;
						}
					}
					else
	...
	int latestStop, num_numAS_minSp;
	int *inds_minSp = (int *)malloc(num_numAS_maxGap * sizeof(int));
	maxWithTies(latestStop, inds_minSp, num_numAS_minSp, spStopIters_maxGap, num_numAS_maxGap);
	int numAS = inds_maxGap[inds_minSp[num_numAS_minSp - 1]] + 1;
	...
	//Output
	std::ofstream fout;
	fName = path + "\\" + datasetName + "_PUSh_train_performance_" + s_seedId + +"_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	fout.open(fName);
	...
	fName = path + "\\" + datasetName + "_PUSh_trainPreLabels_" + s_seedId + "_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	fout.open(fName);
	for (int i = 0; i < numTrain; i++) {
		fout << preLabels[i] << " ";
	}
	...
	fName = path + "\\" + datasetName + "_PUSh_shInfo_" + s_seedId + "_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	fout.open(fName);
	for (int i = 0; i < maxNumSh; i++) {
		for (int j = 0; j < 3; j++) {
			fout << shInfo[3 * i + j] << " ";
	...
	fName = path + "\\" + datasetName + "_PUSh_shTransTrainTss_" + s_seedId + "_" + s_maxNumSh + "_" + s_minNumIters + "_" + s_maxNumIters + ".txt";
	fout.open(fName);
	fout.precision(15);
	for (int i = 0; i < numTrain; i++) {
